...
phl_sales <- read_csv("~/Desktop/FW 25/MUSA 5080/opa_properties_public.csv", show_col_types = FALSE)
```
...
```{r}
coefs <- coef(model4)[-1]
coefs <- coefs[!grepl("\\^2|:", names(coefs))]

# Get top 10 features by absolute coefficient value
top10 <- sort(abs(coefs), decreasing = TRUE)[1:10]

# Create ranking table
feature_importance <- data.frame(
  Rank = 1:10,
  Feature = names(top10),
  Coefficient = round(as.numeric(top10), 4)
)

print(feature_importance)

#The most important feature is EconKDE. It has the largest coefficient compared for all the other features. This reflects that being close to commerical activities is really important to housing prices. There is also a pattern that spatial features are important.
```
