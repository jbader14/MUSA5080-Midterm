...
Our final model's is able to explain 63.5% of the variation on *logged* house prices in Philadelphia, with an average error of 48.66%. Our residuals mostly fall along the Q-Q plot's line and are homoscadestic (as seen in the Scale-Location plot), meaning that the residuals both fall on a normal distribution and the variance of residuals is consistent across all independent variable values. Our K-folds analysis returned an R2 of .6822 (higher than the model's original .635), illustrating our model's efficacy with non-test data. The feature that matters most is the economic density around the examined housing unit, as it first has a negative relationship with housing price and then has a positive relationship.
