...
phl_sales <- read_csv("C:/Users/wensh/Desktop/opa_properties_public.csv", show_col_types = FALSE)
```
...
#### Load Philly Census Data from Previously Retrieved Files (not sure if still work after switching to census tract)
...
input_pbf <- "D:/ppa midterm/us-northeast-251022.osm.pbf"
...
# Extract the 5 smallest distances for each point
nearest_5 <- apply(dist_matrix, 1, function(x) sort(x)[1:5])

# Get the average or total distance if needed
mean_nearest5 <- apply(dist_matrix, 1, function(x) mean(sort(x)[1:5]))

phl_sales_final_sf_final$mean_5nn_dist <- mean_nearest5
...
```{r}
# 1️⃣ Drop geometry and identifier columns
phl_sales_final_sf_final2=phl_sales_final_sf_final%>%
  mutate(econkde=EconKDE)%>%
  mutate(treekde=TreeKDE)
summary_data <- phl_sales_final_sf_final2 %>%
  st_drop_geometry() %>%
  select(-c(
...
# 3️⃣ Generate summary table with stargazer
stargazer(summary_numeric,
          type = "text",           # "text" for console, "latex" or "html" for export
