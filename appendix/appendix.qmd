...
phl_sales <- read_csv("C:/Users/wensh/Downloads/opa_properties_public.csv", show_col_types = FALSE)
```
...
if ("other_tags" %in% names(poi)) {
  ...
  stop("❌ No 'other_tags' column found in the shapefile.")
}
...
```{r}
poi=st_read("data/filtered poi/philadelphia_poi_filtered.shp")
```
...
###Tree density
```{r}
trees=st_read("data/ppr_tree_inventory_2024/ppr_tree_inventory_2024.shp")
...
philly_neighborhoods=philly_neighborhoods%>%
  st_transform(2272)
philly_bg_sf$GEOID=as.numeric(philly_bg_sf$GEOID)
philly_blockgroup_final <- philly_bg_sf %>%
  left_join(philly_blockgroup, by = "GEOID")
philly_blockgroup_final=philly_blockgroup_final%>%
  st_transform(2272)
#merge them together
phl_sales_clean_sf_final=phl_sales_clean_sf%>%
  st_join(philly_blockgroup_final,join=st_within)%>%
...
### Kernel Density Rasters (Tree density)
